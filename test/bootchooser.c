...
");
	...
	/* check firmware.0 is not considered as primary (i.e. not booted) */
	g_assert_null(r_boot_get_primary(NULL));

	/* check firmware.0 and firmware.1 cannot be set to good */
	g_assert_false(r_boot_set_state(firmware0, TRUE, NULL));
...
boot_partition=3\n\
"));
	g_assert_false(r_boot_set_state(firmware1, TRUE, NULL));
	g_assert_true(test_raspberrypi_reboot_tag(fixture, "0"));
	g_assert_true(test_raspberrypi_autoboot_txt(fixture, "\
[all]\n\
tryboot_a_b=1\n\
...
"));
	...
	/* check firmware.0 and firmware.1 cannot be set to primary */
	g_assert_false(r_boot_set_primary(firmware0, NULL));
	g_assert_true(test_raspberrypi_reboot_tag(fixture, "0"));
	g_assert_true(test_raspberrypi_autoboot_txt(fixture, "\
[all]\n\
tryboot_a_b=1\n\
...
boot_partition=3\n\
"));
	g_assert_false(r_boot_set_primary(firmware1, NULL));
	g_assert_true(test_raspberrypi_reboot_tag(fixture, "0"));
	g_assert_true(test_raspberrypi_autoboot_txt(fixture, "\
[all]\n\
tryboot_a_b=1\n\
...
");
	...
	/* check firmware.0 is considered good (i.e. not booted but tryboot) */
