...
static void config_file_emmc_boot_linked_no_parent_test(ConfigFileFixture *fixture, gconstpointer user_data)
{
	g_autoptr(RaucConfig) config = NULL;
	GError *ierror = NULL;
	g_autofree gchar* pathname = NULL;
...
device=/dev/mmcblk0p1\n";
	...
	pathname = write_tmp_file(fixture->tmpdir, "emmc-boot-linked-no-parent.conf", contents, NULL);
	g_assert_nonnull(pathname);

	res = load_config(pathname, &config, &ierror);
	g_assert_error(ierror, R_CONFIG_ERROR, R_CONFIG_ERROR_PARENT);
	g_assert_cmpstr(ierror->message, ==, "emmc-boot-linked slot 'bootloader.0' must have a parent");
	g_assert_false(res);
	g_clear_error(&ierror);
}

static void config_file_emmc_boot_linked_different_devices(ConfigFileFixture *fixture, gconstpointer user_data)
{
	g_autoptr(RaucConfig) config = NULL;
	GError *ierror = NULL;
	g_autofree gchar* pathname = NULL;
...
bootname=system0\n";
	...
	pathname = write_tmp_file(fixture->tmpdir, "emmc-boot-linked-different-devices.conf", contents, NULL);
	g_assert_nonnull(pathname);

	res = load_config(pathname, &config, &ierror);
	g_assert_error(ierror, R_CONFIG_ERROR, R_CONFIG_ERROR_INVALID_DEVICE);
	g_assert_cmpstr(ierror->message, ==, "bootloader slot 'bootloader.0' and associated parent 'rootfs.0' are on differring devices");
	g_assert_false(res);
	g_clear_error(&ierror);
}
...
bootname=system0\n";
	...
	g_assert_cmpstr(ierror->message, ==, "Need at least 2 valid emmc-boot-linked group slots, but only found 1");
	g_assert_false(res);
	g_clear_error(&ierror);
}
...
bootname=system1\n";
	...
	res = load_config(pathname, &config, &ierror);
	g_assert_false(res);
	g_assert_error(ierror, R_EMMC_ERROR, R_EMMC_ERROR_FAILED);
	g_clear_error(&ierror);
}
...
int main(int argc, char *argv[])
{
	...
	g_test_add("/config-file/emmc-boot-linked-invalid-device", ConfigFileFixture, NULL,
			...
			config_file_fixture_tear_down);
	g_test_add("/config-file/emmc-boot-linked-no-parent", ConfigFileFixture, NULL,
			config_file_fixture_set_up, config_file_emmc_boot_linked_no_parent_test,
			config_file_fixture_tear_down);
	g_test_add("/config-file/emmc-boot-linked-differnt_devices", ConfigFileFixture, NULL,
			config_file_fixture_set_up, config_file_emmc_boot_linked_different_devices,
			config_file_fixture_tear_down);
