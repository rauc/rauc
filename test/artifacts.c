...
static void artifacts_fixture_set_up(ArtifactsFixture *fixture,
...
static void artifacts_fixture_tear_down(ArtifactsFixture *fixture,
		gconstpointer user_data)
{
	g_autofree gchar *tree_cmd = g_strdup_printf("tree -a %s", fixture->tmpdir);
...
static RArtifact* create_random_artifact(const gchar *tmpdir, const gchar *name, gsize size, const guint32 seed)
{
	...
	pathname = g_strdup_printf("%s/.artifact-%s-%s", tmpdir, name, artifact->checksum.digest);
...
static void test_find(ArtifactsFixture *fixture, gconstpointer user_data)
{
	...
	/* create artifact */
	artifact = create_random_artifact(fixture->repo->path, "a1", 64, 21799804);
...
static void test_create_load(ArtifactsFixture *fixture,
		gconstpointer user_data)
{
	g_autofree gchar *tree_cmd = g_strdup_printf("tree -a %s", fixture->tmpdir);
	g_autoptr(GError) error = NULL;
	g_autofree gchar *a_filename = NULL;
	g_autofree gchar *l_filename = NULL;
	RArtifact *artifact = NULL;
	RArtifact *artifact_dup = NULL;
	gboolean res = FALSE;
	...
	a_filename = g_strdup_printf("%s/.artifact-a1-%s", fixture->repo->path, artifact->checksum.digest);
	l_filename = g_strdup_printf("%s/a1", fixture->repo->path);
	...
	/* add a link */
	g_ptr_array_add(artifact->references, (gpointer)g_intern_static_string(""));
	...
	g_ptr_array_remove(artifact->references, (gpointer)g_intern_static_string(""));
	...
	artifact_dup = create_random_artifact(fixture->repo->path, "a1", 64, 21799804);
...
static void test_create_load_parent(ArtifactsFixture *fixture,
		gconstpointer user_data)
{
	g_autofree gchar *tree_cmd = g_strdup_printf("tree -a %s", fixture->tmpdir);
	g_autoptr(GError) error = NULL;
	...
	a_filename = g_strdup_printf("%s/.artifact-a1-%s", fixture->repo->path, artifact->checksum.digest);
	l_filename = g_strdup_printf("%s/rootfs.0/a1", fixture->repo->path);
	...
	/* add a link */
	g_ptr_array_add(artifact->references, (gpointer)g_intern_static_string("rootfs.0"));
	...
	/* remove the link again */
	g_ptr_array_remove(artifact->references, (gpointer)g_intern_static_string("rootfs.0"));
