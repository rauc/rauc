...
static void composefs_recurse_objects(GHashTable *objects, struct lcfs_node_s *node)
{
	...
	for (size_t i = 0; i < lcfs_node_get_n_children(node); i++) {
		...
		composefs_recurse_objects(objects, child);
	}
}
...
static gboolean composefs_objects_from_image(GHashTable *objects, const gchar *image_path, GError **error)
{
	...
	composefs_recurse_objects(objects, node);
	...
	while ((entry_name = g_dir_read_name(dir))) {
		...
		while ((inner_entry_name = g_dir_read_name(inner_dir))) {
			...
			if (!g_file_test(full_object_path, G_FILE_TEST_IS_REGULAR)) {
				g_message("Unexpected data in artifact repo: %s", full_object_path);
				continue;
			}
...
gboolean r_artifact_repo_prune_composefs(RArtifactRepo *repo, GError **error)
{
	...
	while (g_hash_table_iter_next(&iter, (gpointer*)&object_name, NULL)) {
		...
		g_debug("Failed to find required object '%s' in local object store", object_name);
		g_hash_table_iter_remove(&iter);
		missing++;
	}
	if (missing)
		g_warning("Failed to find %d required objects in local object store", missing);
	...
	while (g_hash_table_iter_next(&iter, (gpointer*)&object_name, NULL)) {
		...
		if (unlink(object_path) == -1) {
			...
			g_set_error(error,
					G_FILE_ERROR,
					g_file_error_from_errno(err),
					"Failed to remove unused object '%s': %s", object_name, g_strerror(err));
			return FALSE;
		}
	...
	g_info("Removed %d unused objects in local object store", removed);
	...
	while ((entry_name = g_dir_read_name(dir))) {
		...
		if (rmdir(full_inner_path) == -1) {
			...
			g_set_error(error,
					G_FILE_ERROR,
					g_file_error_from_errno(err),
					"Failed to remove unused object: %s", g_strerror(err));
			return FALSE;
		}
	}
...
gboolean r_composefs_artifact_install(const RArtifact *artifact, const RaucImage *image, const gchar *name, GError **error)
{
	...
	if (removed)
		g_message("Skipping copy of %d existing objects for image %s\n", removed, image->filename);

	g_message("Need to get %d new objects from bundle", g_hash_table_size(image_objects));
	...
	for (guint i = 0; i < image_objects_sorted->len; i++) {
		...
		if (!copy_file(bundle_object_store_path, object_name, local_object_store_path, object_name, &ierror)) {
			g_propagate_prefixed_error(error, ierror, "failed to copy object: ");
			return FALSE;
		}
