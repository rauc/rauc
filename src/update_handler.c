...
static gboolean archive_to_ext4_handler(RaucImage *image, RaucSlot *dest_slot, const gchar *hook_name, GError **error)
{
	...
	/* emptyfs does not have an image included. So there's nothing to extract */
...
static RaucImageTypeMap image_type_map[] = {
	...
	//{"raw-caibx", "nand", img_to_nand_handler}, /* unsupported */
 ...
 * image types. The mappings must be ordered with more specific patterns first
 * (e.g. '*.squashfs-*.caibx' before '*.squashfs-*') to avoid selecting the wrong image type." */
