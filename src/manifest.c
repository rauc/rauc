...
{
	/* It is fine to not have a 'type' only when using custom install hooks */
	...
	if (!derived_type) {
		g_set_error(error, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_INVALID_VALUE,
				...
				image->filename ? image->filename : "(no filename)");
		return FALSE;
	}
	...
	if (g_strcmp0(image->type, "emptyfs") == 0) {
		if (has_filename) {
			g_set_error(error, R_MANIFEST_ERROR, R_MANIFEST_PARSE_ERROR,
					"Slot type 'emptyfs' does not support setting a file name.");
			return FALSE;
		}
	} else {
...
static gboolean parse_image(GKeyFile *key_file, const gchar *group, RaucImage **image, GError **error)
{
	...
	if (!iimage->artifact) {
		...
		if (g_error_matches(ierror, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND)) {
			g_clear_error(&ierror);
			/* Attempt to derive image type from filename when not explicitly specified */
