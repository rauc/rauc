...
/* Check that a digest string consists only of the hex characters expected for
 * its checksum type. The digest is used to build paths (below the artifact
 * repository and below the slot data directory), so anything else must be
 * rejected here.
 */
static gboolean is_valid_digest(const gchar *digest, GChecksumType type)
{
	gssize expected_len = g_checksum_type_get_length(type) * 2;

	if (expected_len <= 0)
		return FALSE;

	if (strlen(digest) != (gsize)expected_len)
		return FALSE;

	for (gssize i = 0; i < expected_len; i++) {
		if (!g_ascii_isxdigit(digest[i]))
			return FALSE;
	}
...
/* Check whether path contains '..' as a component, which would allow escaping
 * the directory it is resolved against. */
static gboolean has_parent_dir_component(const gchar *path)
{
	g_auto(GStrv) components = g_strsplit(path, "/", -1);

	for (gchar **c = components; *c != NULL; c++) {
		if (g_strcmp0(*c, "..") == 0)
			return TRUE;
	}
...
static gboolean check_manifest_bundled(const RaucManifest *mf, GError **error)
{
	...
	/* The hook filename is joined onto the bundle content dir and the result is
	 * executed, so a '..' component would let it escape the content dir. */
	if (mf->hook_name && has_parent_dir_component(mf->hook_name)) {
		g_set_error(error, R_MANIFEST_ERROR, R_MANIFEST_CHECK_ERROR, "Hook filename %s must not contain '..'", mf->hook_name);
		return FALSE;
	}
	...
	for (GList *l = mf->images; l != NULL; l = l->next) {
		...
		if (!is_valid_digest(image->checksum.digest, image->checksum.type)) {
			g_set_error(error, R_MANIFEST_ERROR, R_MANIFEST_CHECK_ERROR, "Invalid digest %s for image %s", image->checksum.digest, image->filename);
			return FALSE;
		}
...
gboolean check_manifest_input(const RaucManifest *mf, GError **error)
{
	...
	if (mf->hook_name && has_parent_dir_component(mf->hook_name)) {
		g_set_error(error, R_MANIFEST_ERROR, R_MANIFEST_CHECK_ERROR,
				"Hook filename %s must not contain '..'", mf->hook_name);
		return FALSE;
	}
