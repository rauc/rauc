...
gboolean load_config(const gchar *filename, RaucConfig **config, GError **error)
{
	...
	c->ignore_slot_as_seed = g_key_file_get_boolean(key_file, "casync", "ignore-slot-seed", &ierror);
	if (g_error_matches(ierror, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND) ||
	    g_error_matches(ierror, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_GROUP_NOT_FOUND)) {
		c->ignore_slot_as_seed = FALSE;
		g_clear_error(&ierror);
	} else if (ierror) {
		g_propagate_error(error, ierror);
		res = FALSE;
	...
	g_key_file_remove_key(key_file, "casync", "ignore-slot-seed", NULL);
