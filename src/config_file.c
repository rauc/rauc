...
static gboolean parse_polling_section(GKeyFile *key_file, RaucConfig *c, GError **error)
{
	...
	c->polling_candidate_criteria = key_file_consume_string_list(key_file, "polling", "candidate-criteria", supported_polling_candidate_criteria, &ierror);
	if (ierror) {
		g_propagate_error(error, ierror);
		return FALSE;
	} else if (!c->polling_candidate_criteria) {
		c->polling_candidate_criteria = g_strdupv((GStrv)default_polling_candidate_criteria);
	}

	c->polling_install_criteria = key_file_consume_string_list(key_file, "polling", "install-criteria", supported_polling_install_criteria, &ierror);
	if (ierror) {
		g_propagate_error(error, ierror);
		return FALSE;
	}

	c->polling_reboot_criteria = key_file_consume_string_list(key_file, "polling", "reboot-criteria", supported_polling_reboot_criteria, &ierror);
	if (ierror) {
		g_propagate_error(error, ierror);
		return FALSE;
	}

	c->polling_source = key_file_consume_string(key_file, "polling", "source", NULL);
	if (!c->polling_source) {
		g_set_error(
				error,
				R_CONFIG_ERROR,
		...
		return FALSE;
	} else if (!g_str_has_prefix(c->polling_source, "http")) {
		g_set_error(
				error,
				R_CONFIG_ERROR,
				R_CONFIG_ERROR_POLLING,
