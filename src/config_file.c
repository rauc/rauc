...
static const gchar *supported_poll_candidate_criteria[] = {
	"higher-semver",
	"different-version",
	NULL
};
static const gchar *default_poll_candidate_criteria[] = {
	"higher-semver",
	NULL
};
static const gchar *supported_poll_install_criteria[] = {
	"always",
	"higher-semver",
	"different-version",
	NULL
};
static const gchar *supported_poll_reboot_criteria[] = {
	"updated-slots",
	"updated-artifacts",
	"failed-update",
	NULL
...
static gboolean parse_poll_config(RaucConfig *config, GKeyFile *key_file, GError **error)
{
	GError *ierror = NULL;

	if (!g_key_file_has_group(key_file, "poll"))
		return TRUE;
	...
	config->poll_inhibit_files = key_file_consume_string_list(key_file, "poll", "inhibit-files", NULL, &ierror);
	if (ierror) {
		g_propagate_error(error, ierror);
		return FALSE;
	}

	config->poll_candidate_criteria = key_file_consume_string_list(key_file, "poll", "candidate-criteria", supported_poll_candidate_criteria, &ierror);
	if (ierror) {
		g_propagate_error(error, ierror);
		return FALSE;
	} else if (!config->poll_candidate_criteria) {
		config->poll_candidate_criteria = g_strdupv((GStrv)default_poll_candidate_criteria);
	}

	config->poll_install_criteria = key_file_consume_string_list(key_file, "poll", "install-criteria", supported_poll_install_criteria, &ierror);
	if (ierror) {
		g_propagate_error(error, ierror);
		return FALSE;
	}

	config->poll_reboot_criteria = key_file_consume_string_list(key_file, "poll", "reboot-criteria", supported_poll_reboot_criteria, &ierror);
	if (ierror) {
		g_propagate_error(error, ierror);
		return FALSE;
	}

	config->poll_source = key_file_consume_string(key_file, "poll", "source", NULL);
	if (!config->poll_source) {
		g_set_error(
				error,
				R_CONFIG_ERROR,
				R_CONFIG_ERROR_POLL,
				"Polling source must be set if [poll] section exists"
				);
		return FALSE;
	} else if (!g_str_has_prefix(config->poll_source, "http")) {
		g_set_error(
				error,
				R_CONFIG_ERROR,
				R_CONFIG_ERROR_POLL,
	...
	gint interval_sec = key_file_consume_integer(key_file, "poll", "interval-sec", &ierror);
	if (g_error_matches(ierror, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND)) {
	...
	gint max_interval_sec = key_file_consume_integer(key_file, "poll", "max-interval-sec", &ierror);
	if (g_error_matches(ierror, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND)) {
	...
	config->poll_interval_ms = interval_sec * 1000;
	config->poll_max_interval_ms = max_interval_sec * 1000;

	config->poll_reboot_cmd = key_file_consume_string(key_file, "poll", "reboot-cmd", NULL);

	if (!check_remaining_keys(key_file, "poll", &ierror)) {
		g_propagate_error(error, ierror);
		return FALSE;
	}
...
gboolean load_config(const gchar *filename, RaucConfig **config, GError **error)
{
	...
	/* parse [poll] section */
	if (!parse_poll_config(c, key_file, &ierror)) {
		g_propagate_error(error, ierror);
		return FALSE;
	}
...
void free_config(RaucConfig *config)
{
	...
	g_free(config->poll_source);
	g_strfreev(config->poll_inhibit_files);
	g_strfreev(config->poll_candidate_criteria);
	g_strfreev(config->poll_install_criteria);
	g_strfreev(config->poll_reboot_criteria);
	g_free(config->poll_reboot_cmd);
