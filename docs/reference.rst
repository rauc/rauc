...
``install-criteria`` (optional, default is empty)
  ...
  * ``always``: Any candidate should be installed automatically.
  ...
  .. note:: If you do **not** configure ``install-criteria``, or if no
    conditions are met, a new bundle recognized as a candidate will **not** be
    installed automatically and will require explicit confirmation.
...
*last-success-time* variant ``t`` <FIXME>:
    The time of the last successful polling attempt (in FIXME)
