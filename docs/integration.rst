...
.. important:: The bootloader backend for the Raspberry Pi firmware cannot
   ...
   Its one-shot reboot flag is similar to the EFI variable ``BootNext`` of the
   EFI bootloader backend.
   It is like an implementation boot attempts counter of 1 after an update; the
   firmware cannot fall back to the other slot if the primary slot becomes
   unbootable.
...
A typically ``autooboot.txt`` file booting the first slot looks like this:
...
Consequently, a kernel change implies an update of both filesystems.
...
.. important:: The firmware filesystems contain the kernel command-line in file
   ...
   and thus, the slot specific rootfs filesystem via the kernel parameter
   ``root=``. The bundle must be slot agnostic, and thus, it must **not** set
   that parameter.

Ideally, the firmware filesystem should use an initramfs to mount the root
filesystem guessed from the device-tree blob at startup.
...
The post-installation hook script should be similar to:
...
.. code-block:: sh
  ...
  then
    if [ "$RAUC_SLOT_DEVICE" = "/dev/mmcblk0p2" ]
    then
